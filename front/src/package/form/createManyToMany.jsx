...
import {getColumnsFromModel} from '../lib/tools.js'
import loadTabData from './loadTabData'
import * as modes from '../_util/_mode.js'

export default {
  mixins: [loadTabData],
  methods: {
    createManyToMany({item}) {
          ...
          _action: ({row}) => {
            return (
              <el-link icon={'el-icon-delete'} underline={false} type={'danger'}
                       onClick={() => this.handleRemoveManyToMany(row, item)}></el-link>
            )
          }
        }
          ...
          <div>
            <div {...{style: {marginBottom: '10px'}}}>
              <Button type="primary" disabled={this.readonly}
                      onClick={() => this.handleAddManyToMany(item)} v-permission:create_resource={resourcePath}><i
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          this.modelSelection[item.prop].forEach(sel => {
            ...
            const index = this.formModel[item.prop].findIndex(m => m[item.primaryKey] === sel[item.primaryKey])
            if (index < 0) {
              this.formModel[item.prop].push(sel)
            } else {
              this.formModel[item.prop][index]._mode = modes.CREATE
            }
    ...
    handleRemoveManyToMany(row, item) {
