package cn.know.act.proton.config.security.jwt;

import cn.know.act.proton.core.config.Properties;
...
public class TokenProvider implements InitializingBean {
    ...
    public TokenProvider(Properties properties, UserJpaRepository userJpaRepository) {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        if (!StringUtils.isEmpty(secret)) {
            log.warn("Warning: the JWT key used is not Base64-encoded. " +
                    "We recommend using the `proton.security.authentication.jwt.base64-secret` key for optimum security.");
    ...
    public String createToken(Authentication authentication, boolean rememberMe, boolean saveAuthInToken) {
        ...
        if (saveAuthInToken) {
            ...
            return Jwts.builder()
                    .setId(((User) authentication.getPrincipal()).getId().toString())
    ...
    public Authentication getAuthentication(String token) {
        ...
        if (claims.get(AUTHORITIES_KEY) != null && StringUtils.hasText(claims.get(AUTHORITIES_KEY).toString())) {
            ...
            Optional<User> user = userJpaRepository.findById(UUID.fromString(claims.getId()));
            if (!user.isPresent()) {
                log.error("User id {} not exists.", claims.getId());
                return null;
            }

            return new UsernamePasswordAuthenticationToken(user.get(), token, user.get().getAuthorities());
        }
    }
